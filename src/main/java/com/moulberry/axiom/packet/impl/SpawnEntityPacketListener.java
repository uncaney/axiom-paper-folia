...
public class SpawnEntityPacketListener implements PacketHandler {
    ...
    public void onReceive(Player player, RegistryFriendlyByteBuf friendlyByteBuf) {
        ...
        for (SpawnEntry entry : entries) {
            ...
            // Folia: dispatch the actual spawn to the destination chunk's owning region.
            Bukkit.getRegionScheduler().execute(this.plugin, bukkitWorld, blockPos.getX() >> 4, blockPos.getZ() >> 4,
                () -> spawnOne(player, serverLevel, entry, position));
        }
    }

    private void spawnOne(Player player, ServerLevel serverLevel, SpawnEntry entry, Vec3 position) {
